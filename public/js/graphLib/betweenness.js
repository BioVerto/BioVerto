(function(g5) {
    ...
    algo: function(g) {
            ...
            for(var k in g.connectedComponentsNodes){   //kth component/subgraph of the entire graph will henceforth be accessed as g.connectedComponentNodes[k]
                ...
                betwnss = new Float32Array(nc);
                next = new Float32Array(nc*nc); for (j=0; j<nc*nc; j++) next[j] = Number.POSITIVE_INFINITY; // there's Array.prototype.map() available for prefilling with infinities, but thats actually slower
                dist = constructAdjacencyMatrix(currentNodes, currentEdges); //construct adjacency matrix for a particular component, cause this is where we're gonna get real performance gains
